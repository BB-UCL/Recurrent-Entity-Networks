...
class EntityNetwork():
    ...
    def __init__(self, emb_dim, vocab_size, num_slots, max_story_len, max_sent_len, optimiser=lasagne.updates.adam):
        ...
        self.Queries = T.itensor3(name='Queries')  # Num_stories X Num_queries X K_max
        self.Indices = T.ivector(name="Indices")  # Num_Stories X Num_queries
        self.Answers = T.imatrix(name='Answers')  # Num_stories X Num_queries

        # Data Set dimensions
        self.N = T.shape(self.Stories)[0]
        self.L = max_story_len
        self.K = max_sent_len
    ...
    def _get_answer(self, h_T, queries, hop_wt, out_wt):
        ...
        u = T.sum(attn.dimshuffle([0, 1, 'x'])*h_T, axis=2)  #(N*N_q, num_slots)
    ...
    def _create_network(self, params):
        ...
        self.init_keys = init*one_repeats  # shape (N_stories, num_slots, emb_dim)
        ...
        self.H_s = T.reshape(self.H_s, [-1, self.num_slots, self.emb_dim], ndim=2)
        ...
        self._answers = self._get_answer(self.H_s, self.masked_queries,
                                        params['hop_weight'],
                                        params['out_weight'])  # (N, N_q, D)

        # Define the loss function and get the accuracy
        self.loss = T.sum(T.nnet.categorical_crossentropy(self._answers,
                                                          self.Answers.flatten()))

        self.accuracy = T.mean(T.eq(T.argmax(self._answers, axis=1), self.Answers))
...
if __name__ == "__main__":
    ENT = EntityNetwork(10, 160, 15, 10, 7)
