...
class EntityNetwork():
    ...
    def _create_network(self, params):
        ...
        self.accuracy = T.mean(T.eq(T.argmax(self._answers, axis=1), self.Answers.flatten()), dtype='float64')

    def _initialise_weights(self):
        params = {}
        params['emb_matrix'] = self._init_weight([self.vocab_size, self.emb_dim])
        params['mask'] = theano.shared(np.ones([self.K, self.emb_dim]).astype('float64'))
    ...
    def _init_weight(self, shape, name=None, scale=0.1):
        return theano.shared((scale*np.random.normal(size=shape)).astype('float64'), name=name)
